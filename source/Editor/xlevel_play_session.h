...
namespace xlevel
{
    ...
    {
        if (State.m_PlayState == level_state::play_state::Paused) State.m_PlayState = level_state::play_state::Playing;
    }
    ...
    inline std::string RequestPlay( level_context& Ed ) noexcept
    {
        auto& State = Ed.State();
    ...
    inline std::string RequestPause( level_state& State ) noexcept
    {
        if (State.m_PlayState != level_state::play_state::Playing) return "Pause: not playing";
        State.m_PlayState = level_state::play_state::Paused;
        return "Paused";
    }

    // One frame. From Paused: one tick, stays Paused. From Stopped: starts Play, runs the first tick, lands Paused.
    inline std::string RequestStep( level_context& Ed ) noexcept
    {
        auto& State = Ed.State();
        using play_state = level_state::play_state;
    ...
    // the Play-mode design notes for the full research/design). Deduplicated by
    ...
    // level_state::m_PendingKeepTweaksCommands (xlevel_editor.h, needed by the confirmation
    ...
    //---------------------------------------------------------------------------
    inline std::string RequestStop(level_context& Ed, std::optional<bool> KeepOverride) noexcept
    {
        auto& State = Ed.State();
        if (State.m_PlayState == level_state::play_state::Stopped) return "Stop: already stopped";
