...
// guarded since BuildGamePluginIfStale (LevelEditor_GamePluginBuild.h) logs from a background thread while
// the main thread renders it every frame. Meant to be included via the umbrella
// (LevelEditor_GamePlugin.h) only, after <mutex>/<vector>/<string> and ImGui are already available.
