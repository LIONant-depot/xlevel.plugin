...
namespace xlevel::commands
{
    ...
    //================================================================================================
    struct add_scene_dependency_cmd : level_command
    {
        add_scene_dependency_cmd(xundo::system& System, void* pDataBase) noexcept : level_command(System, "AddSceneDependency", pDataBase) { RegisterArguments(); }
    ...
    //================================================================================================
    struct remove_scene_dependency_cmd : level_command
    {
        remove_scene_dependency_cmd(xundo::system& System, void* pDataBase) noexcept : level_command(System, "RemoveSceneDependency", pDataBase) { RegisterArguments(); }
        const char* getCommandHelp() const noexcept override
        {
        ...
        std::string Redo() noexcept override
        {
            ...
            else
            {
                ...
                for (auto& Hit : Hits)
                {
                    const auto Target = xscene::commands::ResolvePropertyTarget(LevelContext(), SceneGuid, Hit.m_HolderId, Hit.m_ComponentGuid);
                    ...
                    xscene::commands::RecordPropertyOverride(LevelContext(), Target, SceneGuid, Hit.m_HolderId, Hit.m_Path, ValueStr);
                }
            }
        ...
        void Undo(xundo::undo_file& File) noexcept override
        {
            ...
            if (bHadClear)
            {
                for (auto& H : Hits)
                {
                    ...
                    const auto BeforeEntity = xscene::commands::ResolveEntityReferenceTarget(LevelContext(), BeforeSceneGuid, static_cast<xecs::scene::permanent_id>(H.BeforeId));
                    const auto Target = xscene::commands::ResolvePropertyTarget(LevelContext(), SceneGuid, static_cast<xecs::scene::permanent_id>(H.HolderId), H.Comp);
                    if (!Target.m_pInfo) continue;
                    ...
                    const auto Len = xscene::commands::FormatPropertyValue(Buffer, AnyVal);
                    const std::string ValueStr(Buffer.data(), Len > 0 ? static_cast<std::size_t>(Len) : 0);
                    xscene::commands::RecordPropertyOverride(LevelContext(), Target, SceneGuid, static_cast<xecs::scene::permanent_id>(H.HolderId), H.Path, ValueStr);
                }
            }
        }
...
#endif // XLEVEL_COMMANDS_SCENE_DEPENDENCY_H